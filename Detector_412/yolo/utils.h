...
namespace utils 
{
    ...
    void show(const std::vector<std::vector<Defect>>& objectss, const std::vector<std::string>& classNames,
        const int& cvDelayTime, std::vector<cv::Mat>& imgsBatch);

    void draw(const std::vector<std::vector<Defect>>& objectss, std::vector<cv::Mat>& imgsBatch, std::vector<std::string>& uuid, bool save=false);
