...
void utils::show(const std::vector<std::vector<Defect>>& defects, const std::vector<std::string>& classNames,
	const int& cvDelayTime, std::vector<cv::Mat>& imgsBatch)
{
	...
	for (size_t bi = 0; bi < imgsBatch.size(); bi++)
	{
		if (!defects.empty())
		{
			for (auto& defect : defects[bi])
			{
				if (classNames.size() == 91) // coco91
				{
					color = Colors::color91[defect.box.label];
				}
				if (classNames.size() == 80) // coco80
				{
					color = Colors::color80[defect.box.label];
				}
				if (classNames.size() == 20) // voc20
				{
					color = Colors::color20[defect.box.label];
				}
				cv::rectangle(imgsBatch[bi], cv::Point(defect.box.left, defect.box.top), cv::Point(defect.box.right, defect.box.bottom), color, 2, cv::LINE_AA);
				cv::String det_info = classNames[defect.box.label] + " " + cv::format("%.4f", defect.box.confidence);
				bbox_points[0][0] = cv::Point(defect.box.left, defect.box.top);
				bbox_points[0][1] = cv::Point(defect.box.left + det_info.size() * 11, defect.box.top);
				bbox_points[0][2] = cv::Point(defect.box.left + det_info.size() * 11, defect.box.top - 15);
				bbox_points[0][3] = cv::Point(defect.box.left, defect.box.top - 15);
...
void utils::draw(const std::vector<std::vector<Defect>>& defects, std::vector<cv::Mat>& imgsBatch, std::vector<std::string>& batchuuid, bool save) {
	cv::Scalar color = cv::Scalar(0, 255, 0);
	cv::Point bbox_points[1][4];
	const cv::Point* bbox_point0[1] = { bbox_points[0] };
	int num_points[] = { 4 };
	for (size_t bi = 0; bi < imgsBatch.size(); bi++) {
		if (!defects.empty()) {
			for (auto& defect : defects[bi])
			{
				color = Colors::color80[defect.box.label];
				cv::rectangle(imgsBatch[bi], cv::Point(defect.box.left, defect.box.top), cv::Point(defect.box.right, defect.box.bottom), color, 2, cv::LINE_AA);
				cv::String det_info = defect.defectName + " " + cv::format("%.4f", defect.box.confidence);
				bbox_points[0][0] = cv::Point(defect.box.left, defect.box.top);
				bbox_points[0][1] = cv::Point(defect.box.left + det_info.size() * 11, defect.box.top);
				bbox_points[0][2] = cv::Point(defect.box.left + det_info.size() * 11, defect.box.top - 15);
				bbox_points[0][3] = cv::Point(defect.box.left, defect.box.top - 15);
				...
				//if (!defect.box.land_marks.empty())
				//{
				//	for (auto& pt : box.land_marks)
				//	{
				//		cv::circle(imgsBatch[bi], pt, 1, cv::Scalar(255, 255, 255), 1, cv::LINE_AA, 0);
		...
		if (save) {
			assert(batchuuid.size() == imgsBatch.size());
			std::string tempPath = "images/";
			auto uuid = batchuuid[bi];
			cv::imwrite(tempPath + uuid + ".jpg", imgsBatch[bi]);
		}
	}
