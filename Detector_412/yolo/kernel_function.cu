...
__global__ 
void gray2rgb_device_kernel(const unsigned char* grayImage, float* rgbImage, int batch_size, int img_volume) {
	int dx = blockDim.x * blockIdx.x + threadIdx.x;
	int dy = blockDim.y * blockIdx.y + threadIdx.y;
    ...
    if (dx < img_volume && dy < batch_size) {
        ...
        rgbImage[rgbIndex] = grayImage[grayIndex];       // Red
        rgbImage[rgbIndex + 1] = grayImage[grayIndex];   // Green
        rgbImage[rgbIndex + 2] = grayImage[grayIndex];   // Blue
    }
}
...
__global__
void norm_device_kernel(unsigned char* src, float* dst,
	int batch_size, int img_height, int img_width, int img_area, int img_volume,
	float scale,
	float mean0, float mean1, float mean2,
	float std0, float std1, float std2)
...
void normDevice(const int& batchSize, unsigned char* src, int srcWidth, int srcHeight,
	float* dst, int dstWidth, int dstHeight,
	utils::InitParameter param)
{
	dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
